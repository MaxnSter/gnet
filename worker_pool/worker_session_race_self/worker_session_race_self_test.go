...
func TestNewPoolRaceSelf(t *testing.T) {
	...
	raceFunc := func(sId int) {
		...
		raceF := func(ctx worker_pool.Context) {
			if ts.race != nil {
		...
		raceF1 := func(ctx worker_pool.Context) {
			ts.race = nil
			nwg.Done()
		}
...
func TestPoolRaceSelf_Stop(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		idx := i
		q.Put(&tSession{int64(idx), nil}, func(ctx worker_pool.Context) {
...
func TestPoolRaceSelf_Stop2(t *testing.T) {
	...
	for i := 0; i < 500000; i++ {
		wg.Add(1)
		idx := i
		q.Put(&tSession{int64(idx), nil}, func(_ worker_pool.Context) {
