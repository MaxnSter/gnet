...
func TestNewPoolRaceSelf(t *testing.T) {
	...
	raceFunc := func(sId int) {
		...
		raceF := func(ctx gnet_context.Context) {
			if ts.race != nil {
		...
		raceF1 := func(ctx gnet_context.Context) {
			ts.race = nil
			nwg.Done()
		}
...
func TestPoolRaceSelf_Stop(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		idx := i
		q.Put(&tSession{int64(idx), nil}, func(ctx gnet_context.Context) {
...
func TestPoolRaceSelf_Stop2(t *testing.T) {
	...
	for i := 0; i < 500000; i++ {
		wg.Add(1)
		idx := i
		q.Put(&tSession{int64(idx), nil}, func(_ gnet_context.Context) {
