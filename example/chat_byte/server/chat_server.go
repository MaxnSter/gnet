...
func onClose(s gnet.NetSession) {
	msg := fmt.Sprintf("user:%s, leave", s.Addr().String())
...
func onMessage(ev gnet.Event) {
	s := ev.Session()
	msg := fmt.Sprintf("user:%s, talk:%s", s.Addr().String(),
		ev.Message())
...
func main() {
	...
	module := gnet.NewModule(gnet.WithCoder("byte"), gnet.WithPacker("raw"),
	...
	server := gnet.NewNetServer("ws", "chat", module, operator)
	server.ListenAndServe("0.0.0.0:2007")
}
