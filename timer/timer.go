...
func newTimerManager(pool pool.Pool) *timerManager {
	tm := &timerManager{
		pool:        pool,
...
func (tm *timerManager) AddTimer(expire time.Time, interval time.Duration, cb OnTimeOut) Cancel {
	t := tm.get()
